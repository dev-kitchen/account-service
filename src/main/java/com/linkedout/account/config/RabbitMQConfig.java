...
public class RabbitMQConfig {
	...
	public Queue apiQueue() {
		return new Queue(RabbitMQConstants.ACCOUNT_API_QUEUE, false);
	}

	@Bean
	public Binding apiBinding(Queue apiQueue, DirectExchange apiExchange) {
		return BindingBuilder.bind(apiQueue).to(apiExchange).with(RabbitMQConstants.ACCOUNT_API_ROUTING_KEY);
	}

	// 소비자
	@Bean
	public Queue consumerQueue() {
		return new Queue(RabbitMQConstants.ACCOUNT_SERVICE_CONSUMER_QUEUE, false);
	}

	@Bean
	public Binding consumerBinding(Queue consumerQueue, DirectExchange serviceExchange) {
		return BindingBuilder.bind(consumerQueue).to(serviceExchange).with(RabbitMQConstants.ACCOUNT_CONSUMER_ROUTING_KEY);
	}

	// 리스너
	@Bean
	public Queue listenerQueue() {
		return new Queue(RabbitMQConstants.ACCOUNT_SERVICE_LISTENER_QUEUE, false);
	}

	@Bean
	public Binding responseBinding(Queue listenerQueue, DirectExchange serviceExchange) {
		return BindingBuilder.bind(listenerQueue).to(serviceExchange).with(RabbitMQConstants.ACCOUNT_LISTENER_ROUTING_KEY);
	}
