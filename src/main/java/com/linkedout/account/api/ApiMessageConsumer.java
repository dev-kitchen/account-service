 ...
 * 메시지를 처리합니다. 수행되는 작업은 수신 메시지의 operation 필드에 의해 정의되며, 작업 유형에 따라 {@link AccountService}의 적절한 메서드로
 * 비즈니스 로직을 위임합니다.
 *
 ...
 *
 * <p>의존성: - {@link RabbitTemplate} : 응답 메시지 전송용 - {@link AccountService} : 계정 기반 작업 수행용 - {@link
            ...
            () -> {
              ...
              try {
                return switch (requestKey) {
                  //							case "GET /api/recipes/health" ->
                  //								recipeService.health(request, response).thenReturn(response);
                  default -> {
